...
| Tabela de prazos e SLAs | Provavelmente parte do próprio Regimento ou de uma normativa interna separada | Mesma incerteza do regimento | Coordenação acadêmica | **Sem acesso.** Mesma pendência do Regimento — se vier como capítulo dele, resolve-se no mesmo pedido; se for normativa à parte, mesma ação: solicitar via professor da disciplina ou secretaria |
